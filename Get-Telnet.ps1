...
{   Param (
    ...
    # telnet 失敗例外處理 by cfwang 20151118
    Catch 
     {
      ...
      Write-host $errormsg
            ...
            If ($Socket.Connected)             #當socket 建得起來才做下面的事情
            {   $Stream = $Socket.GetStream()
                ...
                ForEach ($Command in $Commands)
                         {  $Writer.WriteLine($Command) 
                            $Writer.Flush()
                            write-host ($Command) #Print command log to CLI 
                ...
                {   $Read = $Stream.Read($Buffer, 0, 1024) 
                    write-host ($Encoding.GetString($Buffer, 0, $Read)) #Print log to CLI
        ...
        Else     
        { 
           Write-Warning "無法連到設備: $($RemoteHost):$Port"
           Add-content ( "ConnectFailedList.txt" ) -value "$($RemoteHost):$Port - $errormsg"  #Unable connect to host 's logging to ConnectFailedList.txt
...
<# 
 修改為可大批 telnet CPE by cfwang 2015/11/18
 #>

clear-host
$CPEhosts = Get-Content .\hosts.txt
$numcount=1
write-host "準備處理 "  $CPEhosts.count  "台 CPE 設定囉 ^.<"
write-host ""

pause "Press any key to continue"
foreach ($CPEhost in $CPEhosts)
{
write-host "* 開始設定 $numcount/"($CPEhosts.count) " CPE - IP:$CPEhost  *"
Get-Telnet -Commands  (Get-Content .\script.txt) -RemoteHost "$CPEhost" -OutputPath .\$CPEhost.txt -WaitTime 500
$numcount++
}
write-host "做完囉 ^.< "
write-host ""